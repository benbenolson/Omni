...
class UPDFDeviceBlitter : public DeviceBlitter
{
public:
                        UPDFDeviceBlitter         (PrintDevice              *pDevice);
   virtual             ~UPDFDeviceBlitter         ();
   ...
   friend std::ostream& operator<<                (std::ostream&             os,
                                                   const UPDFDeviceBlitter&  device);

private:
   bool                 updfMonoRasterize         (PBYTE                     pbBits,
                                                   PBITMAPINFO2              pbmi,
                                                   PRECTL                    prectlPageLocation,
                                                   BITBLT_TYPE               eType);
   bool                 updfColorRasterize        (PBYTE                     pbBits,
                                                   PBITMAPINFO2              pbmi,
                                                   PRECTL                    prectlPageLocation,
                                                   BITBLT_TYPE               eType);
