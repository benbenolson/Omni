 ...
 */
#ifndef _Star_ESC_Blitter
#define _Star_ESC_Blitter
...
class Star_ESC_Blitter : public DeviceBlitter
{
public:
                         Star_ESC_Blitter              (PrintDevice             *pDevice);
   virtual              ~Star_ESC_Blitter              ();
   ...
   friend std::ostream&  operator<<                     (std::ostream&            os,
                                                         const Star_ESC_Blitter& device);

private:
   bool                  starMonoRasterize             (PBYTE                    pbBits,
                                                         PBITMAPINFO2             pbmi,
                                                         PRECTL                   prectlPageLocation,
                                                         BITBLT_TYPE              eType);
   bool                  starColorRasterize            (PBYTE                    pbBits,
                                                         PBITMAPINFO2             pbmi,
                                                         PRECTL                   prectlPageLocation,
                                                         BITBLT_TYPE              eType);
