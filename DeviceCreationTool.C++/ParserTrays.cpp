...
start_element(const string &n, const XMLPropertyHash &p)
{
   if (n == "deviceTray")
   {
      pPropertyNode = (TrayProperty *) getTraysHead()->getNewChildNode();
...
end_element(const string &n)
{
   if (n == "deviceTray" )
   {
      getTraysHead()->addChild( pPropertyNode ) ;
   }
   else if (n == "name" )
   {
      pPropertyNode->setTrayName( charsRead );
   }
   else if (n == "trayType" )
   {
      pPropertyNode->setTrayType( charsRead );
   }
   else if (n == "command" )
   {
