...
void ParserOrientations::
start_element(const string &n, const XMLPropertyHash &p)
{
   if (n == "deviceOrientation" )
   {
      pPropertyNode = (OrientationProperty *) getOrientationsHead()->getNewChildNode();
...
end_element(const string &n)
{
   if (n == "deviceOrientation" )
   {
      getOrientationsHead()->addChild(pPropertyNode);
   }
   else if (n == "name")
   {
      pPropertyNode->setOrientationName(charsRead);
   }
