            ...
            GModule **phmodDevice)
{
   ...
   *phmodDevice = g_module_open (pszFullDeviceName, (GModuleFlags)0);
   ...
   if (DebugOutput::shouldOutputDeviceTester ()) DebugOutput::getErrorStream ()       << __FUNCTION__ << ": pfnNewDeviceWArgs = 0x" << std::hex << (uintptr_t)pfnNewDeviceWArgs << std::dec << std::endl;
...
//#define PDC 1
   ...
   if (!pDevice)
   {
      DebugOutput::getErrorStream () << "DeviceTester4::" << __FUNCTION__ << ": the new device failed to open!" << std::endl;
   ...
   if (pDevice->hasError ())
   {
      DebugOutput::getErrorStream () << "DeviceTester4::" << __FUNCTION__ << ": the new device has an error!" << std::endl;
   ...
   if (pDevice)
   {
      ...
      pEnumJPGroups = pDevice->listJobProperties ();
