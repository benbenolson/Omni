 ...
 */
#ifndef _DeviceSheetCollate
#define _DeviceSheetCollate
...
protected:
   ...
   Device     *pDevice_d;
   PSZRO       pszSheetCollate_d;
   int         indexSheetCollate_d;
   BinaryData *pbdData_d;
};

class DefaultSheetCollate : public DeviceSheetCollate
{
public:
   enum {
      DEFAULT_INDEX_SHEET_COLLATE = 2
   };

                               DefaultSheetCollate     (Device                    *pDevice,
                                                        PSZRO                      pszJobProperties);

   DeviceSheetCollate         *create                  (Device                    *pDevice,
                                                        PSZCRO                     pszJobProperties);
   static DeviceSheetCollate  *createS                 (Device                    *pDevice,
                                                        PSZCRO                     pszJobProperties);
   ...
   virtual std::string         toString                (std::ostringstream&        oss);
   friend std::ostream&        operator<<              (std::ostream&              os,
                                                        const DefaultSheetCollate& self);
};
