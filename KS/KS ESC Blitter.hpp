 ...
 */
#ifndef _KS_ESC_Blitter
#define _KS_ESC_Blitter
...
class KS_ESC_Blitter : public DeviceBlitter
{
public:
                         KS_ESC_Blitter              (PrintDevice             *pDevice);
   virtual              ~KS_ESC_Blitter              ();
   ...
   friend std::ostream&  operator<<                     (std::ostream&            os,
                                                         const KS_ESC_Blitter& device);

private:
   bool                  ksMonoRasterize             (PBYTE                    pbBits,
                                                         PBITMAPINFO2             pbmi,
                                                         PRECTL                   prectlPageLocation,
                                                         BITBLT_TYPE              eType);
   bool                  ksColorRasterize            (PBYTE                    pbBits,
                                                         PBITMAPINFO2             pbmi,
                                                         PRECTL                   prectlPageLocation,
                                                         BITBLT_TYPE              eType);
