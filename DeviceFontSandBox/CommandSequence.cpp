 ...
 *   Sept. 25, 2002
 *   $Id: CommandSequence.cpp,v 1.3 2002/10/30 23:48:58 rcktscientist Exp $
 */
 ...
 *
 * @param n  name of this node; this CommandSequence owns this string now
 * @param v  value of this node; this CommandSequence owns this string now
 */
CommandSequence::CommandSequence (const char *n, const char *v):
 ...
 */
CommandSequence::CommandSequence (const CommandSequence &node):
  next(NULL)
{
  //
 ...
 * @param node  the node to copy
 */
CommandSequence & CommandSequence::operator= (const CommandSequence & node)
{
  //
 ...
 */
const char * CommandSequence::copy (const char *string)
{
  if (string == NULL)
    return NULL;
 ...
 */
void CommandSequence::setNext (CommandSequence *anode)
{
  anode->next = next;
  next = anode;
}
 ...
 */
void CommandSequence::print() const
{
  const CommandSequence *cs = this;
  int i=1;
  while (cs != NULL)
    {
      cout << i << ": " << cs->name << " --> " << cs->value << endl;
      i++;
      cs = cs->next;
    }
}
 ...
 */
ostream & operator<< (ostream & s, const CommandSequence & cs)
{
  s << "CommandSequence[\"" << cs.getName() << "\",\"" << 
    cs.getValue() << "\"]";
  return s;
}
...
ostream & DevFont::operator<< (ostream & s, const CommandSequence & cs)
{
    s << "CommandSequence[" << cs.name << " = \"" << cs.value << "\"]";
    return s;
}
}
